...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  const post = fullBlogPosts[resolvedParams.slug];
  if (!post) {
    return {
  ...
  const url = `https://infinitykit.online/blog/${resolvedParams.slug}`;
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const articleSchema = {
    ...
    'mainEntityOfPage': `https://infinitykit.online/blog/${resolvedParams.slug}`
  };
  ...
  const breadcrumbSchema = {
    ...
    'itemListElement': [
      ...
      },
      {
        ...
        'position': 3,
        'name': post.title,
        'item': `https://infinitykit.online/blog/${resolvedParams.slug}`
      }
    ]
  };
  ...
  return (
    <>
      ...
      />
      <BlogClient slug={resolvedParams.slug} />
    </>
  );
}
